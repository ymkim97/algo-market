...
def fetch_test_data(problem_id: int) -> Tuple[List[str], List[str]]:
    ...
    if len(ins) != len(outs):
        raise ValueError(
            f"Mismatched number of input ({len(ins)}) and output ({len(outs)}) files for problem {problem_id}")
...
def _sort_test_files(filenames: List[str]) -> Tuple[List[str], List[str]]:
    def get_test_case_number(filename: str) -> int:
        return int(os.path.splitext(filename)[0].rsplit('-', 1)[-1])
    
    ins = sorted(
        [f for f in filenames if f.endswith('.in')], 
        key=get_test_case_number
    )
    outs = sorted(
        [f for f in filenames if f.endswith('.out')], 
        key=get_test_case_number
    )
