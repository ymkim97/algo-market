...
const ProblemForm: React.FC = () => {
  ...
  const handleImageUpload = async (file: File): Promise<string> => {
    if (!problemId) {
      throw new Error('임시 문제가 아직 생성되지 않았습니다.');
    }
  ...
  const handleTestCasePairUpload = async () => {
    if (!problemId) {
      toast.error('임시 저장 문제가 아직 생성되지 않았습니다.');
      return;
    }
  ...
  const handleSaveDraft = async (e: React.FormEvent) => {
    ...
    if (!problemId) {
      toast.error('임시 저장 문제가 생성되지 않았습니다.');
      return;
    }
  ...
  return (
    <>
      <ProgressBar loading={draftLoading || loading || testCaseUploading} />
      {draftLoading ? (
